...
const PostsList = ({ restaurantId }) => {
  ...
  const restaurantPosts = orderedPosts.filter(
    (posts) => posts.restaurant_id == restaurantId
  );
  ...
  const hasAccommodations = (accommodations) => {
    ...
    const sumOfRatings = Object.values(accommodations).reduce(
    ...
    <div className="restaurantCard" key={post.id}>
      ...
      {hasAccommodations(post.accomodations) && (
        <div>
          ...
          {Object.entries(post.accomodations).map(
            ([allergy, rating]) =>
              rating !== 0 && (
                <div key={allergy}>
                  <p>{allergy} Rating:</p>
